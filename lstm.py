...
class Lstm:
    ...
    def __init__(self, options):
        ...
        self.lstm_U = []
        self.lstm_b = []
        for i in range(options['n_lstm_layers']):
            W = self.init_weight(options['embedding_size'], 4 * options['rnn_size'], name = ('rnnw_' + str(i)))
    ...
    def forward_pass_lstm(self, word_embeddings):
        x = word_embeddings
        output = None
        for l in range(self.options['n_lstm_layers']):
            ...
            for lstm_step in range(self.options['lstm_steps']):
                ...
                i, f, o, new_c = tf.split(lstm_preactive, 4, axis=1)
                i = tf.nn.sigmoid(i)
                f = tf.nn.sigmoid(f)
                o = tf.nn.sigmoid(o)
    ...
    def build_model(self):
        ...
        cross_entropy = tf.nn.softmax_cross_entropy_with_logits(logits=logits, labels=answer, name = 'ce')
